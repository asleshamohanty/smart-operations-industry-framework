...
def generate_optimized_shipments_from_materials(db=None):
    """Generate shipments with optimized weather caching and faster processing"""
    shipments = []
        ...
        for i in range(0, len(materials), batch_size):
            ...
            for material in batch_materials:
                ...
                if cached_weather:
                    ...
                    disruption_score = weather_data.get("disruption_score", 0.0)
                    weather_delay_days = weather_data.get("estimated_delay_days", 0)
                    impact_level = weather_data.get("impact_level", "minimal")
                else:
                    # Use mock weather data for faster response (temporary fix)
                    print(f"Using mock weather data for {material_location} to {project_location}")
                    
                    # Generate mock weather data
                    weather_condition = random.choice(["Clear", "Partly Cloudy", "Cloudy", "Light Rain"])
                    temperature = random.randint(20, 35)
                    disruption_score = random.uniform(0.0, 0.3)  # Low disruption for mock data
